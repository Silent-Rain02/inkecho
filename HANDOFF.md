# InkEcho 接手说明

InkEcho 是一个中文文学作品对话与二次创作工作台。项目当前是无构建步骤的原生前端 + Python 标准库服务端，适合直接在本地继续迭代。

## 本地运行

```bash
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
python3 server.py
```

打开 `http://localhost:5173`。模型密钥只放在本地 `.env`，参考 `.env.example`，不要提交真实密钥。

## 主要文件

- `index.html`：页面结构、对话菜单、角色卡（含人物设定）、场景计划、检查点和模型服务操作入口
- `styles.css`：整体视觉、响应式布局、模板卡片和场景计划交互状态
- `app.js`：项目状态、场景计划、浏览器本地保存、流式对话和全部前端交互
- `server.py`：模型服务适配、请求超时、安全静态资源和 HTTP 路由
- `test_server.py`：服务端配置、路由、安全、流式增量输出和模型请求回归测试
- `test_frontend_contract.py`：前端 HTML id、静态资源和关键控件契约检查
- `README.md`：用户向项目说明

## 服务端接口

- `GET /api/health`：读取当前服务配置、历史预算和请求超时状态，不发起模型请求
- `/api/health` 的 `provider_details` 会返回各服务缺失的配置项名称，不包含密钥或端点值
- `GET /api/models`：读取模型列表；Azure 类型端点只返回配置中的部署名，并标记为非实际探测
- `POST /api/probe`：用最小请求验证当前服务的密钥、端点和模型部署
- `POST /api/summarize`：用当前模型把最近对话整理为可复用的剧情摘要；传入 `summary_target: "scene"` 时返回不超过 600 字的当前场景结果

模型上游的认证、权限、模型不存在、限流、超时和连接失败会被服务端转换为安全的中文提示；响应状态码也会分别反映认证、权限、限流或网关超时，便于定位配置问题。

项目菜单支持全量项目 JSON 备份，也支持从对话菜单导出当前单个项目 JSON；导入时两种格式都会被识别。导入前会确认数量并提示现有项目不会被覆盖。全量备份导入后会按备份里的 `activeProjectId` 恢复当时正在编辑的项目（若该项目不在导入范围，则回退到第一项）。
- `POST /api/chat`：非流式回复
- `POST /api/chat/stream`：SSE 流式回复

支持 `custom_azure`、`ollama`、`openai`、`azure` 和 `compatible`。每个项目会按服务分别记住模型名，办公网端点与本地 Ollama 的配置示例都在 `.env.example`。服务端会过滤 `replace_with_your_key`、`your-deployment-name` 等配置占位符，避免状态误报。

## 当前功能

Markdown 导出会保留重试生成的备选回复，并在对话菜单中明确标注，便于交接和比较不同写法。

作品设定、当前章节 / 场景、可排序且会参与模型上下文的场景计划（目标、已发生结果 / 线索、进度摘要、本幕结果提炼；同一时间只保留一个“进行中”场景）、角色回复一键记为场景结果、场景完成并推进、专注模式下的当前场景提示、参考片段导入、剧情摘要新鲜度提示、可逆的精简上下文模式、上下文预览、角色卡（含人物设定、愿望、秘密和关系）、四套创作模板、续写/改写/独白、安全 Markdown 回复渲染、项目切换与分支、灵感摘录、命名检查点、Markdown 导出、JSON 备份、模型流式生成、停止/重试、多回复版本、对话搜索、模型连接测试和上下文用量提示均已实现。每个项目本地最多保留最近 120 条消息；普通聊天仍遵循精简上下文开关并发送最近 20 条，剧情摘要和本幕结果提炼会把本地保留的完整历史交给服务端，再按历史字数预算整理。作品设定输入采用短暂合并保存，切换项目、导出和离开页面时会立即落盘。前端发送前会与服务端对齐最近 20 条消息及历史字数预算，预览内容可作为排查上下文问题的依据。

项目切换区会显示本地项目数据用量；接近浏览器存储配额或保存失败时，会提示立即导出 JSON 备份。

## 验证

```bash
node --check app.js
python3 -m unittest -q test_server.py test_frontend_contract.py
python3 -m py_compile server.py test_server.py test_frontend_contract.py
```

当前服务端回归测试共 39 项，另有前端契约检查；其中包含流式接口对选定模型、输出预算和增量内容的回归验证、长篇摘要读取旧消息的验证、角色人物设定上下文验证，以及上游认证、限流和超时诊断的状态码验证。上下文预览会同步显示当前角色卡、创作倾向和回复长度，侧栏估算也会计入角色设定，便于核对模型实际会收到的创作条件。GitHub Actions 还会启动本地服务进行首页、健康检查和静态资源白名单烟测。服务端普通聊天会在最近 20 条消息中执行历史预算，摘要任务会读取本地保留的完整历史，默认预算 48000 字，可通过 `INK_ECHO_HISTORY_BUDGET` 调整并优先保留最新内容。发布前检查本地 `.env` 没有被加入版本控制，并确认 GitHub Actions CI 通过。

## 后续可选方向

- 为作品提供更多可复用的创作模板和示例
- 增加更细的上下文压缩与长篇项目管理
- 补充浏览器端自动化测试和视觉回归检查
- 评估是否需要账号同步或云端存储
