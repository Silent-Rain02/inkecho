...
作品设定、当前章节 / 场景、可排序且会参与模型上下文的场景计划、场景完成并推进、参考片段导入、剧情摘要提炼、可逆的精简上下文模式、上下文预览、专注模式、角色卡、四套创作模板、续写/改写/独白、安全 Markdown 回复渲染、项目切换与分支、灵感摘录、命名检查点、Markdown 导出、JSON 备份、模型流式生成、停止/重试、多回复版本、对话搜索、模型连接测试和上下文用量提示均已实现。前端发送前会与服务端对齐最近 20 条消息及历史字数预算，预览内容可作为排查上下文问题的依据。
